...
Once canonical writes are switched on, the old service's write paths stay closed **whatever `COACH_WORKFLOW_ENABLED` says**. A rollout flag turned off during an incident must not resurrect a writer that cannot name a swim: a contract-v3 plan describes a day and a gym, and there is no honest reading of that as one occurrence out of a Tuesday carrying two. An old worker gets `409 upgrade_required` and writes nothing, and the legacy scripts check before they send (plan §8.5).
...
Every request carries `X-Coach-Contract-Version`, and `/contract` answers with `supportedVersions` as well as `version`. A worker declaring an unsupported one is refused with `409 upgrade_required` before it claims anything, which is the difference between a stale clone costing a line of diagnosis and a stale clone costing an attempt. The contract is versioned, and `scripts/coach/workflow.ts` refuses one this checkout was not written for. A routine clones the repository for its skill while the app deploys on its own schedule, so a run fired before a deploy — or one reusing a cached workspace — can follow instructions the server has since renamed past. It now stops on the first contract or context read with a line naming both versions, instead of computing a complete result against fields the service will discard. Bump `COACH_CONTRACT_VERSION` and the routine's next run must be on the default branch again. It gets there by itself: the skill's first instruction, before any endpoint is called, fast-forwards the checkout to `origin/main`, which costs a fresh clone nothing and is the whole fix for a cached workspace. It is `--ff-only` on purpose — a dirty or diverged workspace stops the run and is reported rather than forced, because a run that resolves that by itself is a run that coached from something nobody reviewed. Only the routine's own configuration lives outside this repository; nothing else about the upgrade needs a hand.

The job kinds are `create_program`, `prepare_session`, and `review_program`. A `prepare_session` job has one of two targets. A strength target names the programme slot and the gym, as it always did. An endurance target names one occurrence and the exact prescription revision it was queued against — because a week and a weekday cannot identify a session once two rides can share a Tuesday — and its dedupe key is that identity, so a revision arriving afterwards supersedes the job instead of silently re-aiming it. Daily dispatch prepares today and the next two days; overdue endurance work is never prepared again on its own, because nothing rolls forward.

A programme review returns one `coverage` entry per sport the programme includes. The server reads the programme's own sports — from its days and its occurrences, not from the athlete's sport shortcuts — and refuses a review that leaves one out or invents one. Sparse evidence is a stated hold, not an omission.

Claim state, attempt IDs, leases, source revisions and exact target intent are enforced on the server. Outputs cannot invent their authority. Every pending lifting slot needs one keep/substitute/drop disposition; additions need explicit targets. Machine compatibility, measurement, warm-up and run occurrence are validated against the actual athlete's records.

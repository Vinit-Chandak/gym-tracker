...
Validation after integrating main passed: 577 tests across 88 files, TypeScript, the production build, source lint and formatting of supported changed files. Lint excludes generated `.next-qa` files and local `output` artifacts. Tests cover home-only planning, evidence and cumulative-change limits, memo ownership/revisions and the 3,000-word boundary, legacy effort handling, larger-change approval and separate running distance checks.

Apply migration `0017_coach_evidence.sql` after main's existing `0016_modern_micromax.sql`, deploy the compatible app/server and contract v2, then advance the live routine's pinned checkout to that deployed revision. The Vercel production build already runs `db:deploy` before building the app. The audited live entry point pinned `36564277e66b6c3d61fdc4132b91bf96d1e04408`; changing repository instructions alone does not update it. Reclaim stale jobs against fresh context after rollout, and verify a real home-only preparation and a weekly no-change review. No production database or cloud routine was changed during local validation.

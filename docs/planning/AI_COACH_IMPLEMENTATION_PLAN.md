...
Status: the application workflow is implemented on `codex/complete-coaching-flow` as of September 12, 2026. See [implementation progress](AI_COACH_IMPLEMENTATION_PROGRESS.md) for verification and the live rollout gates that remain. The saved cloud routine and production deployment have not been updated by this branch.
...
| D7  | Keep attached reports for future reviews until the athlete removes them.                                                              | Retain private files, offer removal in coaching setup, and invalidate incompatible pending work when a report is removed. Account deletion cascades to retained files.                                                                                                                                                                   |
| D8  | Wait at least seven days before the first weekly review, then use the selected rest day.                                              | Initialize eligibility from consent time and choose the first eligible 04:00 Asia/Kolkata rest-day boundary.                                                                                                                                                                                                                             |
| D9  | Programme creation and gym replans share three requests per day.                                                                      | Count logical explicit requests together per athlete's local calendar day. Deduplicate repeated submissions and unchanged gym choices; scheduled work does not spend the allowance.                                                                                                                                                      |

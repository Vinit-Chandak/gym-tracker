...
This skill is written for **contract version 5**, the version this checkout ships. Every request declares it, so a server speaking another one answers `upgrade_required` before an attempt is spent, and both commands stop the run. Only then do this clone and the deployed app disagree, and only then are the field names described here not the names being validated — a version that matches is the ordinary case and needs no comment. When they do differ, report the skew and stop. Never work around a rejected field by guessing at another name, stripping a prefix from an identifier, or spending corrections to find the shape by trial: an attempt is worth more than the guess.

The context contains confirmed intake, self-reported baselines, policy, the shared `trainingReference`, retained-report metadata, current structure, equipment/catalogue, pending components, evidence coverage, review interval, prior decisions and `requestsToAddress`. `equipment` and `catalogue` describe the one location named by `equipmentGymId` — the job's own gym where it has one, and the athlete's default gym otherwise, which is what a weekly review gets. Empty lists mean that location genuinely has nothing registered, not that the athlete trains nowhere; say so in `uncertainties` rather than planning as though every exercise were feasible.
...
Require two new comparable training dates for progression; two sets or two jobs do not count twice. A lasting cut needs the server's repeated-decline signal against a retained three-exposure reference, not one poor day. The limits are deliberately asymmetric: chase a good session, never flinch at a bad one. An upward load step may be the percentage or one real increment of that equipment, whichever is larger, so the only jump a light dumbbell offers is never the forbidden one; downward steps keep the plain percentage, and where no small enough cut exists, hold and let a real decline go to review. A rep target may gain up to two reps inside its prescribed range, or go straight to the top of the range when the last two comparable sessions attained it at the prescribed effort; it loses one rep at a time. Read prior changes so daily and weekly decisions do not reuse evidence. The versioned numeric limits are initial engineering defaults, not scientifically established optimal thresholds. Express an athlete's goal in the dose and effort targets you prescribe, never by treating these ceilings as adjustable. Larger changes go to review. Honor `loadConvention` **everywhere, not only at home**: a machine whose convention is `unknown`, `stack_label` or `assistance` is refused any automatic load change at any gym, commercial ones included, and `unknown` is what a machine carries until somebody sets it. Read the convention off the machine in `equipment` before you plan a load on it: where it is one of those three, retain the known load, or send `adjustment: calibration`, or say in `uncertainties` that the convention has to be confirmed. At home also honor `availableLoads`; hold when the next physical increment is too large and propose a feasible variation if needed.

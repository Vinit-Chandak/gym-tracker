...
**Decisions stay decided.** `recentDecisions` says what the athlete did with recent proposals: approved, declined, sent back with a `revisionNote`, or replaced. A declined change is not proposed again before its `doNotProposeAgainBefore` date unless the athlete asks for it again in a new note — the server refuses the result otherwise, naming the change. A proposal sent back for revisions is reworked from its note. One closed as `outdated` was never answered: propose its changes again if they still hold. `rejected` is an older answer recorded before declines and revision requests were told apart: read its headline and changes as context, not as a decline.

**Weeks already trained are not changed.** Weeks before `programPosition.currentCycle` are finished. Write run changes for the weeks still ahead; for a change that continues the block, the server keeps finished weeks exactly as they were whatever you send, and again when the athlete approves it.

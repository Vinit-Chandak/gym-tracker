...
export async function assessSessionEvidence(
  ...
  for (const entry of result.plan.exercises) {
    ...
    if (!slot?.prescription || !slot.lineageId)
      fail("Adding an exercise needs a program proposal for athlete review.");
    ...
    if (temporary && count > p.sets)
      fail("A temporary recovery adjustment cannot add working sets.");
    ...
    if (
      ...
      fail(`${entry.exerciseSlug}: this set change needs athlete review in a program proposal.`);
    const trend = evidence.exerciseTrends
      ...
      .sort((a, b) =>
        (b.observations[0]?.performedAt ?? "").localeCompare(a.observations[0]?.performedAt ?? ""),
      )[0];
    ...
    const recent = receipts.find(
      (change) =>
        (change.kind === "temporary" ? change.before.loads?.length : change.after.loads?.length) &&
        change.unit &&
    ...
    for (const [index, set] of sets.entries()) {
      if (p.type === "reps" && set.rir === null)
        fail(`${entry.exerciseSlug}: include a target RIR for working rep sets.`);
      if (p.type !== "reps" && set.rir !== null)
        fail(`${entry.exerciseSlug}: use RPE for timed/distance work, not RIR.`);
      if (p.type !== "reps" && set.rpe == null)
        fail(`${entry.exerciseSlug}: include target RPE for timed/distance work.`);
      if (p.type === "reps" && p.rir[0] != null && set.rir! < p.rir[0])
        fail("A daily plan cannot increase effort beyond the program's target RIR.");
      if (!temporary && p.type === "reps" && p.rir[1] != null && set.rir! > p.rir[1] + 1)
        fail("A lasting effort reduction needs program review.");
      ...
      if (
        ...
        fail(`${entry.exerciseSlug}: targets outside the program range need a program review.`);
      ...
      const setBaseline = baselineLoads.find((item) => item.index === index)?.load ?? baselineLoad;
      const loadIncreases =
  ...
  if (
    ...
    fail("The total session set change exceeds 20%; prepare a program proposal for review.");
  if (result.plan.run) {
    const run = result.plan.run;
    if (run.rpe === null) fail("Include target RPE for the run.");
    ...
    if (
      ...
      fail("The run effort is outside the program range and needs review.");
    if (
      runPrescription &&
    ...
    if (deltas.some((delta) => Math.abs(delta) > 0.001)) {
      ...
      if (
        ...
        fail("This run change exceeds the automatic limit; retain the baseline or ask for review.");
      ...
      if (
        ...
        fail("The combined run changes over 14 days exceed the automatic limit.");
      const fresh = freshSources(
        evidence,
        scope,
        cited,
      ...
      if (!temporary && fresh.days < 2)
        fail("Changing this run needs two new comparable running dates.");
      changes.push({
        scope,
        runMode: run.mode,
        kind: temporary

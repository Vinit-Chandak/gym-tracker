...
export const COACH_POLICY = {
  ...
  rules: [
    ...
    },
    {
      ...
      tasks: ["create_program", "prepare_session", "review_program"],
      kind: "server_rule",
      rule: "Maintain the memo yourself from Tell the coach notes and training evidence; the athlete never edits memo items. Read memo.notes.pending and recent in time order; newer explicit corrections supersede older statements, but removing a sentence from a later note does not revoke it. Use memory expectedRevision, upsert, removeIds, corrections and reviewedNoteIds. Mark notes reviewed even if nothing merits long-term memory; overflow remains pending for a later job. Keep at most 40 items, 3000 words and 40000 characters, with 2000 characters per item: a ceiling, not a target. Store useful preferences, repeated trends, exercise observations, experiments and decisions. Use sport general/workout/run. Direct self-reports use status reported and sourceQuote {sourceId, text} with an exact quote from an owned note:<uuid> or confirmed intake:<uuid>, included in sourceIds. Stable preferences may have reviewAfter null; temporary reports need a reassessment date. Inferences use observation/hypothesis, evidence IDs and reviewAfter within 56 days. Never use confirmed for coach-written items. Changing/removing athlete-origin or reported items requires corrections [{itemId, sourceId, text}] quoting a newer note that explicitly corrects that item. Preserve unrelated facts. Read current profile basics, confirmed goals/intake, programme, logs and computed evidence separately instead of duplicating them. Review expired/missing-source claims. Skip raw logs, daily session recaps, duplicated metrics, transient bad-day labels, diagnoses and unsupported facts. Legacy overview is unverified context. Never rewrite plan.memo.",
    },
    {

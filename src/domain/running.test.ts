...
describe("weekly run volume", () => {
  it("groups runs into Tuesday–Monday training weeks, newest first", () => {
    expect(weekStart("2026-09-08")).toBe("2026-09-08");
    expect(weekStart("2026-09-13")).toBe("2026-09-08");
    expect(weekStart("2026-09-14")).toBe("2026-09-08");
    expect(weekStart("2026-09-15")).toBe("2026-09-15");
    expect(weekStart("2026-09-07")).toBe("2026-09-01");
    const runs = [
    ...
    expect(weeks).toEqual([
      { weekStart: "2026-09-15", runs: 1, minutes: 25, km: 4.1 },
      { weekStart: "2026-09-08", runs: 2, minutes: 55, km: 8.8 },
    ]);
  });

  it("keeps Monday in the prior training week and rolls over on Tuesday", () => {
    const volumes = weeklyVolumes(
      [
        { startedOn: "2026-09-14", durationSeconds: 1200, distanceMeters: 3000 },
        { startedOn: "2026-09-15", durationSeconds: 1800, distanceMeters: 5000 },
      ],
      "2026-09-15",
    );
  ...
  it("flags a week that already exceeds last week by more than 30%", () => {
    const last = { weekStart: "2026-09-08", runs: 2, minutes: 50, km: 8 };
    expect(volumeSpike({ weekStart: "2026-09-15", runs: 2, minutes: 60, km: 10 }, last)).toBeNull();
    expect(volumeSpike({ weekStart: "2026-09-15", runs: 3, minutes: 70, km: 12 }, last)).toEqual({
      thisWeekMinutes: 70,
      lastWeekMinutes: 50,
      ratio: 1.4,
    });

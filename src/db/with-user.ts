 ...
 *
 * Every statement here is a network round trip, two for a parameterised one (ADR 0030), so the
 * setup is kept to the claims statement plus, for a write, the athlete lock. A read-only
 * transaction is made read-only inside the claims statement rather than by a separate
 * `SET TRANSACTION`: going read-only is allowed at any point in a transaction, and every later
